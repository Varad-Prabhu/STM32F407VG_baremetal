...
int main(void)
{
    ...
    static const char *WeekdayString[] =
    {
        "",
        "Monday",
        "Tuesday",
    ...
    while (1)
    {
        ...
        if (time.seconds != previousSecond)
        {
            ...
            USART2_WriteString("Time : ");

            USART2_Write2Digits(time.hours);
            USART2_WriteChar(':');

            USART2_Write2Digits(time.minutes);
            USART2_WriteChar(':');

            USART2_Write2Digits(time.seconds);

            USART2_WriteString("   Date : ");

            USART2_Write2Digits(date.date);
            USART2_WriteChar('/');

            USART2_Write2Digits(date.month);
            USART2_WriteChar('/');

            USART2_Write2Digits(date.year);

            USART2_WriteString("   Weekday : ");

            if ((date.weekday >= 1U) && (date.weekday <= 7U))
            {
                USART2_WriteString(WeekdayString[date.weekday]);
            }
            else
            {
                USART2_WriteString("Invalid");
            }
