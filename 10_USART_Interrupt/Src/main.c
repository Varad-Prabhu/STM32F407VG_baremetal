...
int main(void)
{
    ...
    USART2_Init();
    NVIC_SetPriority(USART2_IRQn, 1);               /* Set priotiy for USART2 interrupt */
    NVIC_EnableIRQ(USART2_IRQn);                    /* Enabled USART2 interrupt */
    
    USART2_WriteString("USART2 Initialized\r\n");

    while (1)
    { 
        /* USART handling is done in ISR */
    }
