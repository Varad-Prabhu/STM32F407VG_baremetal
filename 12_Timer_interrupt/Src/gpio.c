    ...
    for (uint32_t pin = 0U; pin < 16U; pin++)
    {
        if (GPIO_InitStruct->Pin & (1U << pin))
        {
            ...
            if (GPIO_InitStruct->Mode == GPIO_MODE_AF)
            {
                ...
                uint32_t afr_offset = (pin % 8U) * 4U;
                if (pin < 8U)
                {
                    GPIOx->AFRL &= ~(0xFU << afr_offset);
                    GPIOx->AFRL |= (GPIO_InitStruct->Alternate << afr_offset);
                }
                else
                {
                    GPIOx->AFRH &= ~(0xFU << afr_offset);
                    GPIOx->AFRH |= (GPIO_InitStruct->Alternate << afr_offset);
                }
            }
        }
    }
